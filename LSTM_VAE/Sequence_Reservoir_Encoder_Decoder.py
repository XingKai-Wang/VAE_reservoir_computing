...
class ReserviorEncoder(nn.Module):
    def __init__(self, input_dim, reservoir_dim, num_filters, z_dim):
        ...
        self.convs = nn.Sequential(
            ...
            nn.Conv3d(self.num_input_channels, self.num_filters, padding=1, kernel_size=3, stride=2),  # 32x32 -> 16x16
            nn.LeakyReLU(),
            nn.Conv3d(self.num_filters, self.num_filters, kernel_size=4, stride=2), # 16x16 -> 7x7
            nn.LeakyReLU())

        self.linear = nn.Linear(self.num_filters * 7 * 7, self.z_dim)
    ...
    def forward(self, x):
        ...
        h_e = self.convs(h)
        h_e = self.linear(h_e.view(x.size(0), -1, 20, self.num_filters*7*7))
...
class ReservoirDecoder(nn.Module):
    def  __init__(self, z_dim, num_filters, output_channels):
        ...
        self.linear = nn.Sequential(
            nn.Linear(self.z_dim, self.num_filters * 7 * 7),
            nn.LeakyReLU()
        )

        self.decoder = nn.Sequential(
            # output = (input − 1) × stride − 2 × padding + dilation × (kernel − 1) + output_padding + 1
            nn.ConvTranspose3d(self.num_filters, self.num_filters, kernel_size=4, padding=0, output_padding=0, stride=2), # 7x7 -> 16x16
            nn.LeakyReLU(),
            nn.ConvTranspose3d(self.num_filters, self.num_filters, kernel_size=3, padding=1, output_padding=1,stride=2),  # 16x16 -> 32x32
            nn.LeakyReLU(),
            nn.ConvTranspose3d(self.num_filters, self.output_channels, kernel_size=3, padding=1, output_padding=1, stride=2) # 32x32 -> 64x64
        )

    def forward(self, z):
        x = self.linear(z)
        x = x.reshape([x.size(0), -1, 20, 7, 7])
        x_recon = self.decoder(x).permute(0, 2, 1, 3, 4)
