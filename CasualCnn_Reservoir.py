...
class CasualReservoirEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, num_filters, z_dim, *args):
        ...
        self.Conv1d = nn.Sequential(
            CasualConv1d(in_channels,out_channels,kernel_size=(3,),dilation=2,A=False),
            nn.LeakyReLU()
        )
    ...
    def forward(self, x):
        x = self.Conv1d(x.view(x.shape[0], -1))
...
class CasualReservoirDecoder(nn.Module):
    def __init__(self, z_dim, out_channels, num_filters, **kwargs):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(2*hidden_filters, hidden_filters, kernel_size=(4,), padding=(1,), stride=(2,)),
            nn.LeakyReLU(),
            nn.ConvTranspose2d(hidden_filters, out_channels, kernel_size=(4,), padding=(1,), stride=(2,)),
            nn.Sigmoid()
        )
