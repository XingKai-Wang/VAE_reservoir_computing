...
from CausalCnn_Reservoir import CausalReservoirEncoder
from CausalCnn_Reservoir import CausalReservoirDecoder
from torchvision.utils import save_image

class VAE(nn.Module):
    def __init__(self, model_name, z_dim, num_filters, activation, T, layer_type, *args, **kwargs):
        ...
        if model_name == 'MLP':
            ...
            self.decoder = CNNDecoder(num_output_channels=1, num_filters=num_filters, z_dims=z_dim // 2)
        elif model_name == 'RC':
            ...
            self.encoder = CausalReservoirEncoder(in_channels=1,out_channels=256,num_filters=num_filters,z_dim=z_dim)
            self.decoder = CausalReservoirDecoder(z_dim=z_dim // 2, out_channels=1,num_filters=num_filters)
